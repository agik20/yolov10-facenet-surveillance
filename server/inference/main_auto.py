...
YOLO_IMG_SIZ, YOLO_CONF_THRESHOLD = 512, 0.4
    ...
    def run(self):
        ...
        try:
            while not self.stop_evt.is_set():
                ...
                # Improved terminal output with fixed-width formatting
                if (current_time - self.last_print_time > 0.2 or names != self.last_names):
                    ...
                    names_str = ', '.join(names)[:40].ljust(40)
                    timestamp = time.strftime("%H:%M:%S")
                    
                    # Clear line and print
                    sys.stdout.write('\033[2K\r')  # Clear entire line
                    sys.stdout.write(f"\r[{timestamp}] {YELLOW}[STATUS]{RESET} Camera: {camera_fps_str} | Infer: {infer_fps_str} | Faces: {faces_count_str} | Names: {names_str}")
                    sys.stdout.flush()
